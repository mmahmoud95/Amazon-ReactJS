...
export const SubSubcategory = () => {
  ...
  useEffect(() => {
    ...
    instance.get(`/products/subSubCategory/${SubSubCategoryID}`).then((res) => {
  ...
  for (const sub in subs) {
    if (subs[sub]._id === SubSubCategoryID) {
      subSubName = lang === "en" ? subs[sub].en?.name : subs[sub].ar?.name;
    }
  }
   ...
   <div className=" row  mt-1 py-1 rounded border border-light-subtle shadow">
            <div className="col-sm-10 ">
              <p className="fs-6 ms-5 pt-2">
                ...
                <span className="text-danger fw-bold">{subSubName}</span>
              </p>
            </div>
      ...
      <div className="row mt-2 mb-2">
        <div className="col-lg-2 filter">
          <div>
            {/* <h3>{categoryName} </h3> */}
            <h4 className="mt-3">{t("category.part4")}</h4>{" "}
